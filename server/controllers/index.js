  ...
  */
  try {
    /* We want to find all the dogs in the Dog database. To do this, we need
       ...
       in an object as a parameter that defines the search. In this case, we want
       to find every dog, so we give it an empty object because that will not filter
       out any dogs.
...
const getDogName = async (req, res) => {
  try {
    /* Here we are trying to do the exact same thing we did in host index up
       above. We want to find the most recently added dog. The only difference
       here is that we are using the query .sort() function rather than passing
       the sort in as a part of the 3rd parameter options object. Both work
       functionally the same. We are just seeing that it can be written in
       more than one way.
...
const setDogName = async (req, res) => {
  /* If we look at views/page2.handlebars, the form has inputs for a firstname, lastname,
     breed and age. When this POST request is sent to us, the bodyParser plugin
     we configured in app.js will store that information in req.body for us.
  */
...
const searchDogName = async (req, res) => {
  ...
  */
  ...
  /* If they do give us a name to search, we will as the database for a dog with that name.
     ...
     try/catch in case the database throws an error or doesn't respond.
  */
  /* We want to increase the age of the most recently added dog.
     ...
     recently added dog we want an empty query so that it can find all of the dogs.

     Finally, findOneAndUpdate would just update the most recent dog it finds that
     matches the query (which could be any dog). So we also need to tell Mongoose to
     sort all the dogs in descending order by created date so that we update the
